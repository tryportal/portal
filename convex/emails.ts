...
export const sendChannelInviteEmail = action({
  args: {
    email: v.string(),
    inviteToken: v.string(),
  ...
  handler: async (_, args) => {
    ...
    const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000";
    const inviteUrl = `${appUrl}/channel-invite/${args.inviteToken}`;
    const fromDomain = process.env.RESEND_DOMAIN ?? "tryportal.app";
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: `Portal <noreply@${fromDomain}>`,
        to: [args.email],
        subject: `${args.inviterName} shared #${args.channelName} with you on Portal`,
        html: `
          <div style="font-family: system-ui, -apple-system, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
            <h2 style="font-size: 18px; font-weight: 500; margin-bottom: 8px;">
              You've been invited to #${args.channelName}
            </h2>
            <p style="font-size: 14px; color: #666; margin-bottom: 24px;">
              ${args.inviterName} has shared a channel from ${args.workspaceName} with you on Portal.
            </p>
            <a href="${inviteUrl}" style="display: inline-block; background: #1a1a1a; color: #fff; padding: 10px 24px; text-decoration: none; font-size: 14px; font-weight: 500;">
              Join Channel
            </a>
    ...
    if (!response.ok) {
      const error = await response.text();
      console.error("Failed to send channel invite email:", error);
      return { success: false, error };
    }
